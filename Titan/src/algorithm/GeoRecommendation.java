...
public class GeoRecommendation {
	public List<Item> recommendItems(String userId, double lat, double lon) {
		...
		//using Set to avoid saving repeated categories
		Set<String> allCategories = new HashSet<>(); // step 2
		for (String item : favoriteItems) {
			allCategories.addAll(conn.getCategories(item)); // db queries (category table)
		}

		allCategories.remove("Undefined"); // tune category set, TicketMaster有时会返回undefined的category
		//我们不想要undefined category参与搜索
